...
class LevelMenu(Menu):
    def __init__(self, game, w = 800, h = 600):
        ...
        self.cursor_rect.midtop = (self.w/3 -5 + self.offset, self.h/4 +120)      
    ...
    def call_api(self):
        print("call api")
        url = 'http://localhost:8080/api/v1/level/1?perPage=6&currentPage='+ str(self.current_page)
        data = []
        try:
    ...
    def display_menu(self):  
        ...
        while self.run_display:
            ...
            s = self.h/4 +120
            w = self.w/2
            # index = (self.current_page-1) * self.per_page
            # print("index", self.number_of_elements) 
            ...
            #         if(i%2 != 0):
            #             button = Button(self.game.display,"Level "+self.game.list_level_infor[index-1]['levelName'] + " ("+ moves+ "/"+ str(self.game.list_level_infor[index-1]['minMoves'])+")", self.w/3, s, 200, 50, True)
            #             button.draw(28)
            #         else:
            #             # print("list_level_infor", index, self.list_level_infor)
            #             button = Button(self.game.display, "Level "+self.game.list_level_infor[index-1]['levelName']+ " ("+ moves+ "/"+ str(self.game.list_level_infor[index-1]['minMoves'])+")", self.w/3*2, s, 200, 50, False)
            #             button.draw(28)
            #             s=s+80   
            for i in range(1, self.number_of_elements+1): 
                index = (self.current_page-1) * self.per_page+ i   
                ...
                if self.game.list_level_infor[index-1]['moves'] != None:
                    ...
                    if(i%2 != 0):
                        ...
                        # print("list_level_infor", index, self.list_level_infor)
                        button = Button(self.game.display, "Level "+self.game.list_level_infor[index-1]['levelName']+ " ("+ moves+ "/"+ str(self.game.list_level_infor[index-1]['minMoves'])+")", self.w/3*2, s, 200, 50, True)
                        # button.draw(28)
                        s=s+80    
                else:
                    if(i%2 != 0):
    ...
    def move_cursor(self):
        ...
        if self.game.DOWN_KEY:
            ...
            if self.state % self.per_page == 0 or self.state % self.per_page == self.number_of_elements or self.game.list_level_infor[self.state]['moves'] == None:
                self.cursor_rect.midtop = (self.button_location[1][0]-5 + self.offset, self.button_location[1][1])
                ...
                if self.state in self.button_state:
                    ...
                    self.cursor_rect.midtop = (self.button_location[location][0] -5+ self.offset, self.button_location[location][1])
                    # self.state = self.button_state[self.state + 1]
...
class OptionsMenu(Menu):
    def __init__(self, game, w = 800, h = 600):
        ...
        self.w = w
        self.h = h
        self.musicx, self.musicy = self.w/2 - 30, self.h/4 +120
        self.volumex, self.volumey = self.w/2 -30, self.h/3 + 180
        # self.mythologyx, self.mythologyy = self.w/3*2, self.h/3 +160
        # self.optionsx, self.optionsy = self.w/3, self.h/3*2 +60   
        self.exitx, self.exity = self.w/3*2, self.h/3*2 +60
    ...
    def display_menu(self):
        ...
        while self.run_display:
            ...
            button1 = Button(self.game.display,self.text_button1, self.w/2, self.h/4 +120, 240, 60, True)
            button2 = Button(self.game.display,self.text_button2, self.w/2, self.h/3 + 180, 240, 60, True)
            
            button1.draw(30)
            button2.draw(30)

            if(button1.check_hover() or button2.check_hover()):
                self.cursor_visible = False
            else:
                self.cursor_visible = True  
            self.draw_cursor()
    ...
    def check_input(self):
        ...
        if self.game.LEFT_KEY and self.game.current_volume>=0.05 and self.state == 'Volume':
...
class TextMenu(Menu):
    def __init__(self, game, line_high =25, size =18, x_pos=400, y_post=None, text_lines=text_lines):
        Menu.__init__(self, game)
